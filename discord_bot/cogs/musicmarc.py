...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def regather_stream(cls, source, *, loop):
        ...
        to_run = functools.partial(
            ytdl.extract_info, url=source.data["webpage_url"], download=False
        )
    ...
    async def put_url(self, ctx, *, url, loop, requester):
        ...
        async with ctx.typing():
            source = await YTDLSource.from_url(
                url=url, requester=requester, loop=loop, download=self.download
            )
    ...
    async def player_loop(self):
        ...
        while not self._bot.is_closed():
            ...
            if not self.download:
                ...
                try:
                    source = await YTDLSource.regather_stream(
                        source, loop=self._bot.loop
                    )
                except Exception as e:
                    await self._channel.send(
                        f"Une erreur s'est produite, vérifier votre connexion.\n"
                        f"```css\n[{e}]\n```"
                    )
                    continue
            ...
            self._guild.voice_client.play(
                source,
                after=lambda _: self._bot.loop.call_soon_threadsafe(self.next.set),
            )
            np = await self._channel.send(
...
class marcMusic(commands.Cog):
    ...
    async def queue_info(self, ctx):
        ...
        if not vc or not vc.is_connected():
            return await ctx.send(
                "Je ne suis pas connecté au salon vocal!", delete_after=20
            )
        player = self.get_player(ctx)
        ...
        embed = discord.Embed(
            title=f"Musiques suivantes {len(upcoming)}", description=description
        )
    ...
    async def now_playing_(self, ctx):
        ...
        player.np = await ctx.send(
            f"**Je joue:** `{vc.source.title}` " f"demandé par `{vc.source.requester}`"
        )
    ...
    def extract_urls(self, search):
        ...
        with ytdl_info:
            result = ytdl_info.extract_info(
                url=search, download=False
            )  # We just want to extract the info
        if "entries" in result:
            urls = [
                entry["webpage_url"] for entry in result["entries"] if entry is not None
            ]
        else:
        ...
        if vc:
            ...
            try:
                ...
                raise VoiceConnectionError(
                    f"Connection au salon vocal: <{channel}> expirée."
                )
        else:
            try:
