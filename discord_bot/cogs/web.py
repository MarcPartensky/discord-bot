...
class Web(commands.Cog):
    ...
    async def google_translate(
        self, ctx: commands.Context, source: str, target: str, *, msg: str
    ):
        ...
        text = "\n".join(
            [
                html.unescape(tr["translatedText"])
                for tr in response.json()["data"]["translations"]
            ]
        )
        await ctx.send(text)
    ...
    @commands.command(
        name="fausse-personne",
        aliases=["fake-person", "dude", "guy", "someone", "thispersondoesnotexist"],
    )
    ...
    async def joke(self, ctx: commands.Context):
        ...
        embed = (
            discord.Embed(title=content, color=self.joke_color)
    ...
    @commands.command(
        name="math", aliases=["maths", "mathématiques", "m", "wolfram", "wolfram-alpha"]
    )
    ...
    def embed_google_result(self, result):
        ...
        embed = (
            discord.Embed(title=result.name, color=discord.Color.blue())
    ...
    async def ip(self, ctx: commands.Context, ip: str):
        ...
        response = requests.request("GET", url, headers=headers, params=querystring)
        embed = discord.Embed(
            title=f"Informations sur {ip}", color=discord.Color.dark_grey()
        )
    ...
    async def summarize(self, ctx: commands.Context, *, msg: str):
        ...
        footer = (
            re.search(pattern, d["datePublished"]).group()
    ...
    async def sentiment(self, ctx: commands.Context, *, msg: str):
        ...
        payload = (
            '{ "documents": [  {   "id": "1",   "language": "fr",   "text": "'
            + msg
            + '"  } ]}'
        )
        headers = {
    ...
    async def synonyms(self, ctx: commands.Context, word: str, lang: str = "fr"):
        ...
        embed = discord.Embed(
            title=f"Synonymes de {word}:", color=discord.Color.greyple()
        )
    ...
    async def quote(self, ctx: commands.Context, language="fr"):
        ...
        embed = discord.Embed(title=d["content"], color=discord.Color.orange())
