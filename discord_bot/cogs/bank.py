...
class Bank(commands.Cog):
    ...
    def has_enough_money(
        self, ctx: commands.Context, account, member: discord.Member, money: int
    ):
    ...
    @access.admin
    async def set_money(
        self, ctx: commands.Context, money: int, member: discord.Member = None
    ):
    ...
    @access.admin
    async def win_money(
        self, ctx: commands.Context, money: int, receiver: discord.Member = None
    ):
    ...
    async def loose_money(
        self, ctx: commands.Context, money: int, buyer: discord.Member = None
    ):
    ...
    async def pay(
        ...
        try:
            self.ensure_payment(
                ctx.author, receiver, receiver_account, buyer, buyer_account, money
            )
        except Bank.Error as e:
        ...
        transaction = Bank.Transaction(
            buyer=buyer.id, receiver=receiver.id, money=money, time=time.time()
        )
    ...
    def describe_account(self, account, user: discord.Member = None):
        ...
        if account.withdrawed_time:
            withdrawed_time = datetime.datetime.fromtimestamp(
                int(account.withdrawed_time)
            )
        else:
        ...
        embed = (
            discord.Embed(
                title=f"Compte bancaire de **{user.name}**", color=self.gold_color
            )
    ...
    async def transactions(
        self, ctx: commands.Context, limit: int = None, reverse=True
    ):
        ...
        description = "\n".join(lines)
        embed = discord.Embed(
            title=title, description=description, color=self.gold_color
        )
        return await ctx.send(embed=embed)
    ...
    async def open_account(self, ctx: commands.Context, member: discord.Member = None):
        ...
        if ctx.author != member and not ctx.author.id in masters:
            return await ctx.send(
                "Vous n'êtes pas autorisés à ouvrir le compte de cette personne."
            )
        account = self.accounts[member.id]
        if len(account) > 1:
            if ctx.author == member:
    ...
    async def close_account(self, ctx: commands.Context, member: discord.Member = None):
        ...
        if ctx.author != member and not ctx.author.id in masters:
            return await ctx.send(
                f"Vous n'êtes pas autorisés à fermer le compte de {member.name}."
            )
