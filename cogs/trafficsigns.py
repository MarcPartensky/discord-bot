...
class TrafficSign(commands.Cog):
    ...
    def get_image(self, html_code, category, number):
        ...
        pattern = re.compile(r'<img src="(https:\/\/static.passetoncode\.fr\/img-panneaux\/[^"]+)')
        results = re.findall(pattern=pattern, string=html_code)
        # print(results)
        return results[-1]

    @sign.command(name="afficher", aliases=['a', 'show', 's'])
    async def show(self, ctx:commands.Context, category:str=None, number:int=None):
        """Affiche un panneau."""
    ...
    async def ask_question(self, ctx:commands.Context, category:str=None, number:int=None):
        """Pose une question."""
    ...
    async def quiz(self, ctx:commands.Context, limit:int=None, category:str=None, number:int=None):
    ...
    async def show_raw_score(self, ctx:commands.Context, member:discord.Member=None):
    ...
    async def show_scores(self, ctx:commands.Context):
        ...
        for score in self.scores:
            ...
            member_score = ', '.join([f'{k}: {v}' for k,v in score.items() if k!='_id'])
            line = f"- {member.name}: {member_score}"
            line = (line.replace('true', 'bonne réponse')
