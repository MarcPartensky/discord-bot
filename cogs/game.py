...
class Game(commands.Cog):
    ...
    async def pfc(self, ctx, user_choice, coins:int=2, player:discord.User=None):
        ...
        if ctx.author!=player and not ctx.author.id in masters:
            await ctx.send(f"> Vous n'êtes pas autorisé à jouer à la place de {player.name}")
            return
        l = {"pierre":0, "feuille":1, "ciseaux":2}
        if not user_choice in l:
            return await ctx.send("> Vous devez choisir entre pierre, feuille ou ciseaux.")
        if coins > self.max_coins:
            return await ctx.send(f"> Vous ne pouvez pas miser au delà de **{self.max_coins}** {emoji.coin}.")
        if coins > casino_account.coins:
            return await ctx.send(f"> {player.name} n'a que **{casino_account.coins}** coins ce n'est pas suffisant.")
