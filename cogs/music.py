        ...
        if 'entries' not in processed_info:
            ...
            while info is None:
                try:
                    ...
                    raise YTDLError('Impossible de récupérer de résultat pour `{}`'.format(webpage_url))

        return cls(ctx, discord.FFmpegPCMAudio(info['url'], **cls.FFMPEG_OPTIONS), data=info)

    @classmethod
    async def create_sources(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None):
        loop = loop or asyncio.get_event_loop()
        ...
        if 'entries' not in data:
            ...
            for process_info in process_info_list:
                process_info['webpage_url'] = "https://www.youtube.com/watch?v="+process_info['url']
...
class Music(commands.Cog):
    ...
    def cog_check(self, ctx: commands.Context):
        if not ctx.guild:
            raise commands.NoPrivateMessage('Cette commande ne peut pas être exécutée en conversation privée.')
        return True
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if voter == ctx.voice_state.current.requester:
            ...
            if total_votes >= ctx.voice_state.skip_votes_number:
                ...
                await ctx.send('Le compte de vote est maintenant à **{}/3**'.format(total_votes))
        else:
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                ...
                await ctx.send('Une erreur s\'est produite pendant le traitement de: {}'.format(str(e)))
            else:
                first = True
