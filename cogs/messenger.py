...
class Messenger(commands.Cog):
    ...
    async def messages(self, ctx:commands.Context, *, limit:int=20):
        ...
        messages = self.client.fetchThreadMessages(thread_id=self.threads[ctx.author], limit=limit)
        lines = []
        authors = {}
        for message in reversed(messages):
            if message.author not in authors:
                authors[message.author] = self.client.fetchUserInfo(message.author)[message.author]
    ...
    def embed_thread(self, thread):
        ...
        if str(thread.type) == "ThreadType.GROUP":
            names = [participant.name for participant in self.client.fetchThreadInfo(*thread.participants).values()]
            names = '\n'.join(names)
