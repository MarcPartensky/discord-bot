...
class Basic(commands.Cog):
    ...
    async def emoji_to_id(self, ctx: commands.Context):
        ...
        user, reaction = await self.bot.wait_for(
    ...
    async def language(self, ctx):
        ...
        await ctx.send(
            "https://i.pinimg.com/originals/d9/4b/4e/d94b4e32dc787df8ae7c5d8a95184796.png"
        )
    ...
    async def howgay(self, ctx):
        ...
        embed = discord.Embed(
            title="How gay?", description=description, color=discord.Color(color)
        )
    ...
    async def permissions(self, ctx, user: discord.Member = None, translating=True):
        ...
        embed = discord.Embed(
            title=f"Permissions de {user.name}", color=discord.Color.red()
        )
    ...
    async def full_date(self, ctx):
        ...
        await ctx.send(
            f"Nous sommes le {days[n.weekday()]} {n.day} {months[n.month]} {n.year}",
    ...
    async def heure(self, ctx):
        """Donne l'heure."""
        await ctx.send(
            f'Il est: {datetime.datetime.now().strftime("%X")}',
            delete_after=delete_after_time,
        )
    ...
    async def date(self, ctx):
        """Donne la date."""
        await ctx.send(
            f'Nous sommes le {datetime.datetime.now().strftime("%d/%M/%Y")}',
            delete_after=delete_after_time,
        )
